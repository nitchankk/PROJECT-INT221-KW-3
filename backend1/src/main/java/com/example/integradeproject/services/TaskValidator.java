...
public class TaskValidator {
        ...
        public static void validateNewTask2DTO(NewTask2DTO newTask2DTO) {
            ...
            if (newTask2DTO.getStatus() == null || newTask2DTO.getStatus().trim().isEmpty()) {
                throw new InvalidTaskDataException("Status cannot be null or empty");
            }
            if (newTask2DTO.getStatus().length() > 50) {
                throw new InvalidTaskDataException("Status cannot exceed 50 characters");
            }
            if (errormessage.length() > 0) {
                throw new InvalidTaskDataException(errormessage.toString());
            }
